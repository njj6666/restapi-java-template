    ...
    */
    ...
    $.widget("ui.dropdownchecklist", {
        ...
        _createDropItem: function(index, value, text, checked, disabled, indent) {        	
			...
			var idBase = (self.sourceSelect.prop("id") || "ddcl");
			var id = idBase + index;
            var checkBox;
            ...
            checkBox = checkBox.prop("index", index).val(value);
            ...
            var checkItem = function(e) {
				...
				if (!disabled) {
	                ...
	                if (self.initialMultiple){
		                ...
		                self.sourceSelect.trigger("change", 'ddcl_internal');
	                }else if(!self.initialMultiple && !checked){//For single select if the value is selected already then there won't be any action
	                	checkBox.prop("checked", !checked);
		                self._syncSelected(checkBox);
		                self.sourceSelect.trigger("change", 'ddcl_internal');
	                }
	                if (!self.initialMultiple) {//This is to  close the drop container for single select alone once the user selected some value
	                	self._toggleDropContainer(false);
	                }
	                
				}
            };   
            if (self.initialMultiple) label.click(function(e) {e.stopPropagation()});        
        ...
        _appendItems: function() {
			...
			if(dropContainerDiv.children().length == 0){
				if(dropContainerDiv.hasClass("ui-dropdownchecklist-dropcontainer")){
					dropContainerDiv.removeClass("ui-dropdownchecklist-dropcontainer");
					dropContainerDiv.addClass("ui-dropdownchecklist-dropcontainer-cus");
				}
        ...
        _updateControlText: function() {
            ...
            var wholeText=new Array();
            if(self.initialMultiple)
            	 wholeText = self._formatText(selectOptions, options.firstItemChecksAll, allSelected);
            else
                wholeText = self._formatTextSingle(selectOptions, options.firstItemChecksAll, allSelected);
            var controlLabel = controlWrapper.find(".ui-dropdownchecklist-text");
            controlLabel.text(wholeText[1]);
            var toolTip=$.trim(wholeText[0]);
            if(toolTip!=null) toolTip=toolTip.substring(0,toolTip.length-1);          
            controlLabel.prop("title", toolTip);           
        },
            ...
            if (firstItemChecksAll && allSelected) {
                ...
                if (text.length > 0) {
                    ...
                    if(text.length >28){
                    	...
                    	var upperCaseText=text.match(/[A-Z]/g);
                    	if(upperCaseText!=null && upperCaseText.length>20){
        ...
        _formatTextSingle: function(selectOptions, firstItemChecksAll, allSelected) {
            var text;
           // var completeText="";
            var textArray=new Array();
            if (firstItemChecksAll && allSelected) {
        ...
        _toggleDropContainer: function() {
            ...
            var hide = function() {
                ...
                if (null != instance) {
					...
					instance.dropWrapper.find("input").prop("tabIndex", -1);
                    instance.dropWrapper.drop = false;
                    $.ui.dropdownchecklist.drop = null;
                    $(document).unbind("click", hide);                   
                    $(".ui-dropdownchecklist-dropcontainer").show(); 
                    $(".ui-dropdownchecklist-dropcontainer").unbind("mouseleave", hide);
            ...
            var show = function(instance) {
                ...
                instance.dropWrapper.css({
                    top: instance.controlWrapper.offset().top +( instance.controlWrapper.outerHeight() -5)+ "px",
                    left: instance.controlWrapper.offset().left + "px"
                })
				var ancestorsZIndexes = controlWrapper.parents().map(
					function() {
						var zIndex = $(this).css("z-index");
						return isNaN(zIndex) ? 0 : 10000}
					).get();
                ...
                instance.controlWrapper.find(".ui-dropdownchecklist").toggleClass("ui-dropdownchecklist-active");
				instance.dropWrapper.find("input").prop("tabIndex", 0);
                instance.dropWrapper.drop = true;
                $.ui.dropdownchecklist.drop = instance;
                $(document).bind("click", hide);               
            	$(".ui-dropdownchecklist-dropcontainer").show();
                $(".ui-dropdownchecklist-dropcontainer").bind("mouseleave", hide);
