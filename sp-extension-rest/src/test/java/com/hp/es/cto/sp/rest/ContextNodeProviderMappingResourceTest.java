 ...
 * @author Victor
 */
public class ContextNodeProviderMappingResourceTest {

	private static final Logger logger = LoggerFactory
			.getLogger(ContextNodeProviderMappingResourceTest.class);
	...
	public void setup() {
		...
		SpringPOJOResourceFactory noDefaults = new SpringPOJOResourceFactory(
				ContextNodeProviderMappingResource.class);
	...
	public void testContextNodeProviderMapping() throws URISyntaxException {
		...
		MockHttpRequest request = MockHttpRequest.get("/v1/context-providers");
		...
		request = MockHttpRequest
				.get("/v1/context-providers?context_node_id=ff808081428eb41d0142fa6cfbb0591n");
		request.accept(MediaType.APPLICATION_XML);
		response = new MockHttpResponse();
		dispatcher.invoke(request, response);
		Assert.assertEquals(HttpServletResponse.SC_OK, response.getStatus());
		String respXml = new String(response.getOutput());
		Assert.assertFalse(respXml.contains("context_provider"));

		// add mapping
		request = MockHttpRequest.post("/v1/context-providers");
		request.accept(MediaType.APPLICATION_XML);
		request.contentType(MediaType.MULTIPART_FORM_DATA
				+ "; boundary=----WebKitFormBoundaryxmsrVjZxadPVoUFZ");
		InputStream is = this.getClass().getResourceAsStream(
				"/add_provider_mapping.txt");
		request.content(is);
		response = new MockHttpResponse();
		dispatcher.invoke(request, response);
		...
		// update mapping
		request = MockHttpRequest.put("/v1/context-providers/" + id);
		request.accept(MediaType.APPLICATION_XML);
		request.contentType(MediaType.MULTIPART_FORM_DATA
				+ "; boundary=----WebKitFormBoundaryxmsrVjZxadPVoUFZ");
		is = this.getClass().getResourceAsStream("/update_providermapping.txt");
		request.content(is);
		response = new MockHttpResponse();
		dispatcher.invoke(request, response);
		logger.info(response.toString());
		...
		// delete mapping
		request = MockHttpRequest.delete("/v1/context-providers/" + id);
		request.accept(MediaType.APPLICATION_XML);
		response = new MockHttpResponse();
		dispatcher.invoke(request, response);
		logger.info(response.toString());
