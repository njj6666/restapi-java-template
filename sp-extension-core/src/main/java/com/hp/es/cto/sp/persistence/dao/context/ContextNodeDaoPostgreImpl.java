 ...
 * @author Victor
 */
public class ContextNodeDaoPostgreImpl extends GenericDaoImpl<ContextNode> implements ContextNodeDao {
	/**
	 * {@inheritDoc}
	 */
	...
	public List<ContextNode> findAllByParentIdAndLdapDns(String parentNodeId, List<String> ldapDnList) {
		String sql = "With RECURSIVE ACCESSNODE " + "as (select distinct UUID from CONTEXT_NODE  " + "WHERE UUID not in ( select distinct EG.CONTEXT_NODE_ID  from CONTEXT_NODE_LDAP_MAPPING EG) "
	...
	public List<String> findParentProvidersByNodeId(String nodeId) {
		String sql = "With RECURSIVE CONTEXTNODE as ( " + "select UUID,NAME,PARENT_NODE_ID " + "from CONTEXT_NODE where UUID=:node  " + "union all select A.UUID,A.NAME,A.PARENT_NODE_ID "
	...
	public List<String> findContextPathByNodeId(String nodeId) {
		String sqlStr = "With RECURSIVE CONTEXTNODE as (select UUID,NAME,PARENT_NODE_ID,CONTEXT_META_DATA_ID from CONTEXT_NODE where UUID=:node "
