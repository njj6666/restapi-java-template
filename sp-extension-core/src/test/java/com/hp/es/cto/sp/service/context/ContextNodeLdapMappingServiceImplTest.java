...
public class ContextNodeLdapMappingServiceImplTest {
	@Autowired
	...
	@Test
	public void testFindAndDeletebyNodeId() {	
		ContextNode envAcontextNode = contextNodeService.findById(envAuuid);
		List<ContextNodeLdapMapping> list = contextNodeLdapMappingService.findByNode(envAcontextNode);
		assertEquals(1, list.size());
		assertEquals(LDAP_ENV_A, list.get(0).getLdapDn());
		ContextNode envDcontextNode = contextNodeService.findById(envDuuid);
		list = contextNodeLdapMappingService.findByNode(envDcontextNode);
		assertEquals(0, list.size());
		contextNodeLdapMappingService.deleteByNodeId(envAcontextNode.getUuid());
		list = contextNodeLdapMappingService.findByNode(envAcontextNode);
		assertEquals(0, list.size());
		ContextNode lifeCContextNode = contextNodeService.findById(lifeCuuid);
		list = contextNodeLdapMappingService.findByNode(lifeCContextNode);
		assertEquals(1, list.size());
		assertEquals(LDAP_LIFE_C, list.get(0).getLdapDn());
		contextNodeLdapMappingService.delete(list.get(0));
		ContextNode envBcontextNode = contextNodeService.findById(envBuuid);
		list = contextNodeLdapMappingService.findByNode(envBcontextNode);
		assertEquals(0, list.size());
	}
