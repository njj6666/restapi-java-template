...
public class ContextNodeProviderMappingServiceImplTest {
	@Autowired
	...
	public void testFindAndDeletebyNodeId() {
		...
		List<ContextNodeProviderMapping> list = contextNodeProviderMappingService.findProviderByNode(envAcontextNode);
		assertEquals(1, list.size());
		assertEquals(PROVIDER_ENV_SOLUTION_A, list.get(0).getCsaProviderId());
		list = contextNodeProviderMappingService.findProviderByNode(envDcontextNode);
		assertEquals(0, list.size());
		contextNodeProviderMappingService.deleteByNodeId(envAcontextNode.getUuid());
		list = contextNodeProviderMappingService.findProviderByNode(envAcontextNode);
		assertEquals(0, list.size());
		list = contextNodeProviderMappingService.findProviderByNode(envBcontextNode);
		assertEquals(1, list.size());
		assertEquals(PROVIDER_ENV_B, list.get(0).getCsaProviderId());
		contextNodeProviderMappingService.delete(list.get(0));
		list = contextNodeProviderMappingService.findProviderByNode(envBcontextNode);
		assertEquals(0, list.size());
	}
